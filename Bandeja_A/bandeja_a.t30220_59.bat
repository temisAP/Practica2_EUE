...
set datecmd=%datecmd:/=\%
set exedir=
set jobnt=./bandeja_a.T30220_59
set jobnt=%jobnt:/=\%
set lognt=./bandeja_a.log
set lognt=%lognt:/=\%
set modedir=
...
set DBSDIR=.
set JIDDIR=c:/users/maría/documents/muse/eue/practica2_eue/bandeja_a
set MSC_APP=no
set MSC_AUTH=edu
set MSC_BASE=c:/program files/msc.software/napa_se/2020/nastran
set MSC_DBS=./bandeja_a
set MSC_EXE='c:/program files/msc.software/napa_se/2020/nastran/msc20200/win64i8/analysis.exe'
set MSC_ISHELLPATH=c:/users/maría/documents/muse/eue/practica2_eue/bandeja_a
set MSC_ISHELLEXT=
set MSC_JID=C:\Users\María\Documents\MUSE\EUE\Practica2_EUE\Bandeja_A\bandeja_a.bdf
set MSC_JIDPATH=
set MSC_JIDTYPE=.bdf
set MSC_MEM=527MB
set MSC_OLD=yes
set MSC_OUT=./bandeja_a
set MSC_SCR=no
...
set PWD=C:\Users\María\Documents\MUSE\EUE\Practica2_EUE\Bandeja_A
set PWD=%PWD:/=\%
cd /d %PWD%
...
set mksol700=no
if "%SOL700%" == "yes" (
   if not exist "./bandeja_a.sol700.pth" (
      set mksol700=yes
      echo  "%MSC_BASE%\%MSC_VERSD%\dyna\win64\bin\dytrandmp.py" nastran > ./bandeja_a.sol700.pth
      set  LSTC_VER=""
   )
)
set cmdline=C:\Program Files\MSC.Software\NaPa_SE\2020\Nastran\msc20200\win64i8\nastran.exe Bandeja_A.bdf prog=edu auth=edu bat=no
...
if exist "bandeja_a.perf" "c:/program files/msc.software/napa_se/2020/nastran/bin/msc20200" mscsystem bandeja_a.perf >> %lognt%
if exist "%sdirnt%\bandeja_a.T30220_59.perf" "c:/program files/msc.software/napa_se/2020/nastran/bin/msc20200" mscsystem %sdirnt%\bandeja_a.T30220_59.perf >> %lognt%
@rem
set /a f98 = 0
call :get_file_size  "%sdirnt%\bandeja_a.T30220_59.f98"
if /i %FILE_SIZE% gtr 0 set /a f98 = 1
set /a f99 = 0
call :get_file_size  "%sdirnt%\bandeja_a.T30220_59.f99"
...
if /i %i% neq 0 (
   ...
   if "%ferr%" == "" (
      if /i %f98% neq 0 (
         if /i %f99% neq 0 copy /f /y "%sdirnt%\bandeja_a.T30220_59.f99"+"%sdirnt%\bandeja_a.T30220_59.f98" "%outnt%.acms_out"
         if /i %f99% equ 0 copy /f /y "%sdirnt%\bandeja_a.T30220_59.f98" "%outnt%.acms_out"
      ) else (
         copy /f /y "%sdirnt%\bandeja_a.T30220_59.f99" "%outnt%.acms_out"
      )
   )
)
@rem
for %%f in ("%sdirnt%\bandeja_a*_ts_*.dac") do call :delete_job_file "%%f"
for %%f in (dtout msg config) do call :concat_job_file %%f
for %%f in ("%sdirnt%\bandeja_a.T30220_59.*") do call :delete_job_file "%%f"
...
if exist "bandeja_a.perf" del /f "bandeja_a.perf"
if "%mksol700%" == "yes" (
   if exist ./bandeja_a.sol700.pth del /f ./bandeja_a.sol700.pth
)
...
setlocal
set ext=%~x1
type "bandeja_a_SN_*.%1" > bandeja_a.%1 2>nul
del /f "bandeja_a_SN_*.%1" 2>nul:
>nul findstr "^" "bandeja_a.%1" || del "bandeja_a.%1"
endlocal
goto :eof
@rem
@rem

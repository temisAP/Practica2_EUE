...
set datecmd=%datecmd:/=\%
set exedir=
set jobnt=./4_sine_msc_random_x.T22400_33
set jobnt=%jobnt:/=\%
set lognt=./4_sine_msc_random_x.log
set lognt=%lognt:/=\%
set modedir=
...
set MSC_AUTH=EDU
set MSC_BASE=d:/archivos de programa/msc.software/napa_se/2020/nastran
set MSC_DBS=d:/archivos_de_programa/scratch/4_sine_msc_random_x.T22400_33
...
set MSC_ISHELLEXT=
set MSC_JID=d:/0_andres/1_universidad/muse/eue/practica2_eue/satelite/1/analisis/4_sine_msc_random_x.dat
set MSC_JIDPATH=
set MSC_JIDTYPE=.dat
set MSC_MEM=637MB
set MSC_OLD=yes
set MSC_OUT=./4_sine_msc_random_x
set MSC_SCR=yes
...
set mksol700=no
if "%SOL700%" == "yes" (
   if not exist "./4_sine_msc_random_x.sol700.pth" (
      set mksol700=yes
      echo  "%MSC_BASE%\%MSC_VERSD%\dyna\win64\bin\dytrandmp.py" nastran > ./4_sine_msc_random_x.sol700.pth
      set  LSTC_VER=""
   )
)
set cmdline=D:\Archivos de programa\MSC.Software\NaPa_SE\2020\Nastran\msc20200\win64i8\nastran.exe D:\0_Andres\1_Universidad\MUSE\EUE\Practica2_EUE\Satelite\1\Analisis\4_sine_msc_random_x.dat
...
if exist "4_sine_msc_random_x.perf" "d:/archivos de programa/msc.software/napa_se/2020/nastran/bin/msc20200" mscsystem 4_sine_msc_random_x.perf >> %lognt%
if exist "%sdirnt%\4_sine_msc_random_x.T22400_33.perf" "d:/archivos de programa/msc.software/napa_se/2020/nastran/bin/msc20200" mscsystem %sdirnt%\4_sine_msc_random_x.T22400_33.perf >> %lognt%
@rem
set /a f98 = 0
call :get_file_size  "%sdirnt%\4_sine_msc_random_x.T22400_33.f98"
if /i %FILE_SIZE% gtr 0 set /a f98 = 1
set /a f99 = 0
call :get_file_size  "%sdirnt%\4_sine_msc_random_x.T22400_33.f99"
...
if /i %i% neq 0 (
   ...
   if "%ferr%" == "" (
      if /i %f98% neq 0 (
         if /i %f99% neq 0 copy /f /y "%sdirnt%\4_sine_msc_random_x.T22400_33.f99"+"%sdirnt%\4_sine_msc_random_x.T22400_33.f98" "%outnt%.acms_out"
         if /i %f99% equ 0 copy /f /y "%sdirnt%\4_sine_msc_random_x.T22400_33.f98" "%outnt%.acms_out"
      ) else (
         copy /f /y "%sdirnt%\4_sine_msc_random_x.T22400_33.f99" "%outnt%.acms_out"
      )
   )
)
@rem
for %%f in ("%sdirnt%\4_sine_msc_random_x*_ts_*.dac") do call :delete_job_file "%%f"
for %%f in (dtout msg config) do call :concat_job_file %%f
for %%f in ("%sdirnt%\4_sine_msc_random_x.T22400_33.*") do call :delete_job_file "%%f"
...
if exist "4_sine_msc_random_x.perf" del /f "4_sine_msc_random_x.perf"
if "%mksol700%" == "yes" (
   if exist ./4_sine_msc_random_x.sol700.pth del /f ./4_sine_msc_random_x.sol700.pth
)
...
setlocal
set ext=%~x1
type "4_sine_msc_random_x_SN_*.%1" > 4_sine_msc_random_x.%1 2>nul
del /f "4_sine_msc_random_x_SN_*.%1" 2>nul:
>nul findstr "^" "4_sine_msc_random_x.%1" || del "4_sine_msc_random_x.%1"
endlocal
goto :eof
@rem
@rem
